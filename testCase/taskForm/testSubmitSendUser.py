...
class SubmitSendUser(unittest.TestCase):
    """抄送任务工单"""
    def setUp(self):
        logger.info('*****启动testSubmitSendUser用例*****')
        warnings.simplefilter('ignore',ResourceWarning)
        #读取该sheet页的所有用例数据
        """self.data_list = get_sheet_data_new("test-data.xlsx","testTaskForm")"""


    def test_submit_send_task_form(self):

        """抄送任务工单接口"""
        dependData = DependData()

        dependData.get_depend_data("test_taskForm_11")
        # 请求参数
        """case_data = get_test_data_new(self.data_list,"test_taskForm_11")
        if not case_data:
            logger.info("用例不存在")
